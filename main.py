...
def model_decision_json(user_input: str) -> Dict[str, Any]:
    ...
    try:
        ch = client.chat.completions.create(model=MODEL, messages=messages, response_format={"type": "json_schema", "json_schema": JSON_SCHEMA})
        ...
        try:
            ...
            ch = client.chat.completions.create(model=MODEL, messages=[{"role": "system", "content": SYSTEM + "\nReturn JSON ONLY with keys: direction, confidence_pct, reason, as_of."}, {"role": "user", "content": user_input}])
            return json.loads(ch.choices[0].message.content)
